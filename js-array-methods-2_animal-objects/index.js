...
const indexOfAnimalWithNameLongerFive = animals.indexOf((animal) => animal.name.length > 5);
...
animals.slice().sort((a,b)=> a.weight - b.weight)
.reverse();
console.log(animalsSortedByWeightReversed)

const animalWithWeightMoreThanFivehundredExists = animals.every((animal) => animal.weight > 500);
console.log(animalWithWeightMoreThanFivehundredExists);
// Hint: Filter for Europe first, then check every animal for its weight.
const allAnimalsInEuropeWeighLessThanOnehundred = animals.filter((animal) => animal.continents.includes("Europe"))
...
.reduce((totalWeight, animal) => totalWeight + animal.weight, 0);
